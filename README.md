...
The basic 'tec' tool is used as a wrapper for included programs like `fingerprint`, `compare` and `filter_gff`.
If no arguments are given the help text will be displayed:
...
Preprocessing pipeline. Requires a paired-end reads in fastq format, a library of tranposable elements in fasta format
and a reference genome in fasta format. Fasta files should be pre indexed using BWA. This pipeline also requires that 
BWA and Samtools are installed and on the OS PATH.
...
Dangler reads are mapped against the reference genome and tagged with the ID od their mates transposable element.
