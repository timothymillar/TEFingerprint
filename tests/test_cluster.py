...
from tefingerprint.cluster import IDBCAN, SIDBCAN

class TestIDBCAN:
    """
    Tests for class IDBCAN.
    """
    ...
    def test_sorted_ascending(self, array, answer):
        ...
        """
        assert IDBCAN._sorted_ascending(array) == answer
    ...
    def test_melt_slices(self, slices, melted_slices):
        ...
        """
        npt.assert_array_equal(IDBCAN._melt_slices(slices), melted_slices)
    ...
    def test_subcluster(self):
        ...
        slices = np.array([(2, 6), (3, 7), (8, 12), (12, 16), (13, 17)],
                          dtype=IDBCAN._DTYPE_SLICE)
        npt.assert_array_equal(IDBCAN._subcluster(array, 4, 5), slices)
    ...
    def test_flat_cluster(self):
        ...
        slices = np.array([(2, 7), (8, 12), (12, 17)],
                          dtype=IDBCAN._DTYPE_SLICE)
        npt.assert_array_equal(IDBCAN._cluster(sub_slices, 4, 5), slices)

    @pytest.mark.parametrize("array,slices",
                             [(np.array([], dtype=int),
                               np.array([], dtype=IDBCAN._DTYPE_SLICE)),
                              (np.array([1], dtype=int),
                               np.array([], dtype=IDBCAN._DTYPE_SLICE)),
                              (np.array([1, 2, 3, 4], dtype=int),
                               np.array([], dtype=IDBCAN._DTYPE_SLICE)),
                              (np.array([1, 2, 3, 4, 5], dtype=int),
                               np.array([(0, 5)], dtype=IDBCAN._DTYPE_SLICE))
                              ],
                             ids=['0<n', '1<n', '4<n', '5==n'])
    def test_idbcan_few_reads(self, array, slices):
        """
        Test for method idbcan with small arrays.
        ...
        always be returned.
        """
        npt.assert_array_equal(IDBCAN.idbcan(array, 5, 5), slices)
...
class TestSIDBCAN:
    ...
    def test_core_distances(self):
        ...
        npt.assert_array_equal(SIDBCAN._core_distances(input_array, 10), point_eps)
    ...
    def test_point_eps_hidden_peak(self):
        ...
        npt.assert_array_equal(SIDBCAN._core_distances(input_array, 3), point_eps)
    ...
    def test_fork_epsilon(self, array, min_points, answer):
        ...
        """
        assert SIDBCAN._fork_epsilon(array, min_points) == answer
    ...
    def test_flatten_list(self, nested, flat):
        ...
        to a list.
        """
        assert list(SIDBCAN._flatten_list(nested)) == flat

    @pytest.mark.parametrize("array,min_points,max_eps,answer",
                             [(np.array([], dtype=int), 5, 5, np.array([], dtype=IDBCAN._DTYPE_SLICE)),
                              (np.array([1], dtype=int), 5, 5, np.array([], dtype=IDBCAN._DTYPE_SLICE)),
                              (np.array([1, 2, 3, 4], dtype=int), 5, 5, np.array([], dtype=IDBCAN._DTYPE_SLICE)),
                              (np.array([1, 2, 3, 4, 5], dtype=int), 5, 5, np.array([(0, 5)], dtype=IDBCAN._DTYPE_SLICE)),
                              (np.array([0, 2, 3, 7, 8, 12, 13, 17], dtype=int), 3, 5, np.array([(0, 8)], dtype=IDBCAN._DTYPE_SLICE)),
                              (np.array([0, 2, 3, 7, 8, 12, 13, 16], dtype=int), 3, 5, np.array([(0, 3), (5, 8)], dtype=IDBCAN._DTYPE_SLICE))],
                             ids=['empty', 'single', 'p < min-points', 'p == min-points', 'parent', 'children'])
    def test_sidbcan(self, array, min_points, max_eps, answer):
        """
        Test for method sidbcan with small arrays.
        Method sidbcan should correctly handle an array of length 0 or greater.
        If the length of the array is less than 'n' then an empty array will
        always be returned.
        """
        npt.assert_array_equal(SIDBCAN.sidbcan(array, min_points, max_eps), answer)

    def test_sidbcan_variations(self):
        """
    ...
    def test_sidbcan_initial_parent_support_calculation(self):
        ...
        answer_1 = np.fromiter([(0, 8)], dtype=SIDBCAN._DTYPE_SLICE)
        answer_2 = np.fromiter([(1, 4), (4, 7)], dtype=SIDBCAN._DTYPE_SLICE)

        npt.assert_array_equal(IDBCAN.idbcan(array, 3, 4), answer_1)
        npt.assert_array_equal(IDBCAN.idbcan(array, 3, 3), answer_1)
        npt.assert_array_equal(IDBCAN.idbcan(array, 3, 2), answer_2)

        npt.assert_array_equal(SIDBCAN.sidbcan(array, 3, 4), answer_1)
        npt.assert_array_equal(SIDBCAN.sidbcan(array, 3, 3), answer_2)
        npt.assert_array_equal(SIDBCAN.sidbcan(array, 3, 2), answer_2)

    def test_sidbcan_child_support_calculation(self):
        """
        Test for method sidbcan.
        ...
        answer = np.fromiter([(0, 3), (5, 8)], dtype=SIDBCAN._DTYPE_SLICE)
        npt.assert_array_equal(SIDBCAN.sidbcan(array, 3), answer)

    def test_sidbcan_default_max_eps(self):
        """
        Test for method sidbcan.
        ...
        answer = np.fromiter([(0, 6), (7, 13)], dtype=SIDBCAN._DTYPE_SLICE)
        npt.assert_array_equal(SIDBCAN.sidbcan(array, 3), answer)

    def test_sidbcan_real(self):
        """Test for method sidbcan"""
        input_array = np.array([   0,    0,   60,   61,   61,   61,   76,   78,  122,  122,  141,
                                 183,  251,  260,  260,  263,  263,  267,  267,  288,  288,  295,
                                 300,  310,  310,  317,  317,  334,  334,  335,  338,  338,  338,
                                ...
                                1217, 1234, 1234, 1591, 1620, 1620, 1662, 1686, 1707, 1755, 1828,
                                1828, 1848, 1848, 1848, 1848, 1851, 1851, 1852, 1917], dtype=int)
        answer_slices = np.fromiter([(0, 55), (56, 80), (83, 97)], dtype=SIDBCAN._DTYPE_SLICE)
        npt.assert_array_equal(SIDBCAN.sidbcan(input_array, 10, max_eps=200, min_eps=10), answer_slices)
    ...
    def test_fit(self):
        ...
        Most edge cases should be caught in tests for component methods.
        """
        hudc_object = SIDBCAN(10, max_eps=200, min_eps=10)
        input_array = np.array([   0,    0,   60,   61,   61,   61,   76,   78,  122,  122,  141,
                                 183,  251,  260,  260,  263,  263,  267,  267,  288,  288,  295,
                                 300,  310,  310,  317,  317,  334,  334,  335,  338,  338,  338,
                                 338,  340,  342,  342,  344,  344,  358,  367,  370,  370,  377,
