...
@pytest.mark.parametrize("strings,answer,strand",
                         [([
                           ...
                           [(671, 524, 'readA'), (747, 600, 'readB'), (9, 5, 'readC')],
                           '+')])
def test_parse_sam_strings(strings, answer, strand):
    """
    Test factory for hidden method _parse_sam_strings.
    Tests for parsing sets of forward, reverse and mixed reads.
    """
    query = list(bam_io._parse_sam_strings(strings, strand=strand))
    assert query == answer
