...
class TestFingerprint:
    ...
    def test_fingerprint_empty(self):
        ...
        reverse = StrandReads(np.array([], dtype=StrandReads.DTYPE_READ),
                              ...
                              source='file.bam',
                              strand='-')
        reads = Reads(forward=forward, reverse=reverse)
    ...
    def test_fingerprint_single_starand(self):
        """
        Test for automatic calculation of :class:`Fingerprint`.
        Effectively integration test for :class:`StrandReads`, :class:`Reads`,
        :class:`Fingerprint`, :class:`HUDC` and :class:`GffFeature`.
        """
        # note that the second value for each read is it's tail position and is not used in any calculation
        forward = StrandReads(np.array([(   0, 0, 'Gypsy27_uniqueID'),
